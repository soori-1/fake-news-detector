...
# --- TOP RIGHT CONTROLS (THEME & ADMIN) ---
# Using standard Streamlit columns so it never breaks the layout
...
st.markdown(f"""
    ...
    .main .block-container {{ max-width: 950px; padding-top: 1rem !important; position: relative; z-index: 10 !important; margin: 0 auto; }}

    /* BACKGROUND ANIMATIONS (PUSHED BACK) */
    .bg-streams {{
    ...
    .space-container {{
        ...
        pointer-events: none; z-index: 1 !important; overflow: hidden;
    }}
    ...
    .floating-card {{
        ...
        background: var(--card-bg); border: 2px solid var(--border-color); color: var(--text-color);
        padding: 6px 14px; border-radius: 6px; box-shadow: var(--card-shadow);
        animation: zoomForward linear infinite; opacity: 0;
    }}
    ...
    .fc-1 {{ top: 25%; left: 8%; animation-duration: 16s; animation-delay: 0s; }}
    .fc-2 {{ top: 75%; right: 10%; animation-duration: 20s; animation-delay: 3s; }}
    .fc-3 {{ top: 40%; left: 12%; animation-duration: 18s; animation-delay: 6s; }}
    .fc-4 {{ top: 65%; right: 8%; animation-duration: 22s; animation-delay: 8s; }}
    .fc-5 {{ top: 30%; left: 60%; animation-duration: 17s; animation-delay: 11s; }}

    .tag-real {{ color: #10B981; border: 2px solid rgba(16, 185, 129, 0.4); background: rgba(16, 185, 129, 0.1); padding: 2px 6px; border-radius: 4px; margin-left: 8px; font-size: 0.75rem; }}
    .tag-fake {{ color: #EF4444; border: 2px solid rgba(239, 68, 68, 0.4); background: rgba(239, 68, 68, 0.1); padding: 2px 6px; border-radius: 4px; margin-left: 8px; font-size: 0.75rem; }}

    /* CRISP INPUTS & BUTTONS */
    .stTextArea label {{ display: none; }}
    .stTextArea textarea {{ background-color: var(--input-bg) !important; color: var(--input-text) !important; border: 2px solid var(--border-color) !important; border-radius: 12px !important; font-size: 1.05rem !important; font-weight: 600 !important; padding: 1.5rem !important; text-align: center; box-shadow: var(--card-shadow); transition: all 0.2s ease; }}
    .stTextArea textarea::placeholder {{ color: var(--sub-text) !important; opacity: 0.9; font-weight: 600; }}
    .stTextArea textarea:focus {{ border-color: #FF6B1A !important; outline: none; }}

    .stButton > button {{ background: var(--text-color) !important; color: var(--bg-color) !important; font-weight: 800 !important; font-size: 1.1rem !important; border: none !important; border-radius: 8px !important; padding: 0.7rem 2.5rem !important; margin: 0 auto !important; display: block !important; transition: all 0.2s ease !important; box-shadow: var(--card-shadow); }}
    .stButton > button:hover {{ transform: translateY(-2px); opacity: 0.9; }}

    .glass-card {{ background: var(--card-bg); border: 2px solid var(--border-color); border-radius: 12px; padding: 24px; text-align: center; box-shadow: var(--card-shadow); }}
    .glass-label {{ font-size: 0.8rem; color: var(--sub-text) !important; text-transform: uppercase; margin-bottom: 8px; font-weight: 800; letter-spacing: 0.05em; }}
    .glass-val {{ font-size: 2.5rem; font-weight: 800; color: var(--text-color) !important; }}
    </style>
    ...
    <div class="space-container">
        ...
        <div class="floating-card fc-2">NEW TAX LOOPHOLE EXEMPTS BILLIONAIRES <span class="tag-fake">FAKE</span></div>
        <div class="floating-card fc-3">G20 DRAFTS JOINT SUPPLY CHAIN RESOLUTION <span class="tag-real">REAL</span></div>
        <div class="floating-card fc-4">GOVERNMENT TO SEIZE ALL RETAIL GOLD <span class="tag-fake">FAKE</span></div>
        <div class="floating-card fc-5">TECH EARNINGS EXCEED Q2 EXPECTATIONS <span class="tag-real">REAL</span></div>
    </div>
...
with tab_verify:
    if model is None:
        st.error("Trained pipeline file `fake_news_pipeline.pkl` was not found. Please run `python train.py` first.")
    else:
        ...
        if analyze_btn:
            ...
            if not input_text:
                ...
                with st.spinner("Processing token distributions..."):
                    ...
                    with col_gauge:
                        st.markdown(f"<p style='text-align:center; color:{chart_font_color}; font-size:0.9rem; font-weight:800; text-transform:uppercase;'>Probability Gauge</p>", unsafe_allow_html=True)
                        fig_gauge = go.Figure(go.Indicator(
                            mode="gauge+number", value=real_prob, domain={'x': [0, 1], 'y': [0, 1]},
                            number={'suffix': "%", 'font': {'size': 40, 'color': chart_font_color, 'family': 'Plus Jakarta Sans', 'weight': 'bold'}},
                            gauge={
                                'axis': {'range': [0, 100], 'tickwidth': 3, 'tickcolor': chart_font_color},
                        ...
                        fig_gauge.update_layout(
                            height=280, margin=dict(l=20, r=20, t=20, b=20), paper_bgcolor="rgba(0,0,0,0)", 
                            font=dict(family='Plus Jakarta Sans', color=chart_font_color, size=15)
                        )
                        st.plotly_chart(fig_gauge, use_container_width=True)

                    with col_feat:
                        st.markdown(f"<p style='text-align:center; color:{chart_font_color}; font-size:0.9rem; font-weight:800; text-transform:uppercase;'>Linguistic Feature Weights</p>", unsafe_allow_html=True)
                        try:
                            ...
                            if len(present_indices) > 0:
                                ...
                                fig_bar.update_layout(
                                    ...
                                    yaxis=dict(autorange="reversed", color=chart_font_color, tickfont=dict(family='Plus Jakarta Sans', color=chart_font_color, size=14)), 
                                    xaxis=dict(color=chart_font_color, gridcolor="rgba(128,128,128,0.2)", tickfont=dict(family='Plus Jakarta Sans', color=chart_font_color, size=13)), 
                                    font=dict(family='Plus Jakarta Sans', color=chart_font_color, size=15, weight="bold"), 
                                    showlegend=False
                                )
